...
export default function StoryCards({ activeBeat }: StoryCardsProps) {
  const [filter, setFilter] = useState<Story['category'] | 'all'>('all')
  const [expanded, setExpanded] = useState<string | null>(null)

  let filtered = filter === 'all' ? stories : stories.filter((s) => s.category === filter)
  if (activeBeat) {
    filtered = filtered.filter((s) => s.beat === activeBeat)
  }
  ...
  return (
    <div>
      ...
      {filtered.length === 0 ? (
        <p className="text-center text-sm" style={{ color: 'var(--color-mist)' }}>
          No stories for this beat yet — check back as coverage grows.
        </p>
      ) : (
              ...
              return (
                <motion.article
                  ...
                  className={`glass-light rounded-xl p-5 cursor-pointer transition-all hover:border-aurora/30 ${
                    story.featured && !activeBeat ? 'md:col-span-2' : ''
                  }`}
                ...
                >
                  ...
                  <h3 className={`font-serif text-white ${story.featured && !activeBeat ? 'text-2xl' : 'text-lg'}`}>
                    {story.title}
                  </h3>
